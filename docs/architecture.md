...
| `logging_setup.py` | Attaches a JSON-serialising rotating-file handler that persists WARNING+ logs (including API-error response bodies) to `logs/pipeline.jsonl` on the shared volume (issue #134) |
...
logs/
  pipeline.jsonl                    # rotating JSON log of WARNING+ events, incl. API-error bodies (issue #134)
```
